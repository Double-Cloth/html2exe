...
补充：若使用“本工具打包后的 EXE”执行构建任务，仍需系统可用 `npm`（通常随 Node.js 安装提供），用于在运行时自动安装并调用 electron-builder；若缓存安装失败会回退尝试 `npx`/`npm exec`。
...
- 当以 EXE 运行且本地 `electron-builder` CLI 缺失时，主进程会自动下载 `electron-builder@26.8.1` 到 `.cache/builder/toolchain` 并优先复用。
- 若缓存工具链下载失败，会自动回退到 `npx`/`npm exec`（系统已安装 Node.js 时可用）。
